...
const MongoLog = () => {
  ...
  const defaultMessage = "Server running on port: 27017"; // \nDB Connected....

  // Fetch logs from the backend API every second
  useEffect(() => {
    const fetchLogs = async () => {
      try {
        const response = await axios.get("http://localhost:3538/mongologs");
        setLogs(response.data);
        setLoading(false);
        setError(""); // Clear the error if fetch is successful
      } catch (err) {
        setError("Mongo not Running"); // Error fetching logs.
        setLoading(false);
      }
    };
